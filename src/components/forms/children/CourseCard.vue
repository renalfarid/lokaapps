...
<template>
    <div v-if="isVisible">
      <ul class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <li v-for="(item, idx) in course" :key="idx" class="m-3 px-4 py-5 duration-150 bg-gray-50 hover:cursor-pointer hover:border-white hover:rounded-xl hover:bg-gray-100">
            <div class="flex items-center gap-x-3" @click="selectCourse(item.id)">
              <div class="bg-white w-14 h-14 border rounded-full flex items-center justify-center">
                <img :src="item.thumbnail" class="rounded-full" /></div>
              <div>
